...
import cn.skuu.common.config.RedisConfig;
...
public final class RedisUtil {
    ...
    public void del(Set<String> kSet) {
     ...
     * @return
     */
    public Set<Object> zRortRange(String key, double min, double max) {
    ...
    public boolean setNx(String key, Object value, long period, TimeUnit timeUnit) {
        if (value == null) {
            value = 1;
        }
        try {
            Boolean result = redisTemplate.opsForValue().setIfAbsent(key, value, period, timeUnit);
            if (result != null && result) {
                return true;
            }
        } catch (Exception e) {
            log.error(String.format("error to set nx key : %s", key));
        }
        return false;
    }
